...
export function ColorSetSelector({ onSelectColor, onSelectPair, selectedColor }: ColorSetSelectorProps) {
          ...
          <div className="grid grid-cols-2 gap-2">
            {currentSet?.colors?.map(color => (
              <Button
                key={color.hex}
                variant="outline"
                className={`h-12 relative ${selectedColor === color.hex ? 'ring-2 ring-primary' : ''}`}
